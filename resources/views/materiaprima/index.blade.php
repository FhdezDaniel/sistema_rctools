...
<html>
    <head>
        ...
        <title>RC Tools - Materia prima</title>
    </head>
    ...
    <body class="bg-gray-300">
        ...
        <div class="ml-5 mt-16 mr-20 flex flex-row justify-end">
                ...
                <a href="materiaprima/create" class="text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-[#4285F4]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#4285F4]/55 mr-2 mb-2">
                    ...
                    <span class="ml-2">CREAR NUEVO REGISTRO DE MATERIA PRIMA</span>
                <a>
                @endrole
            ...
            <table class="w-11/12 text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                @foreach ( $materiaprimas as $materiaprima )
                        <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700 uppercase">
                                <th scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $materiaprima->id }}</th>
                                <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $materiaprima->nombre }}</td>
                                <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $materiaprima->descripcion }}</td>
                                <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $materiaprima->existencia }}</td>
                                <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $materiaprima->fecha_registro }}</td>
                                <td>
                                @role(['Admin','Supervisor'])
                                <form action="{{ route ('materiaprima.destroy',$materiaprima->id)}}" method="POST">
                                <a href="/materiaprima/{{ $materiaprima->id }}/edit" class="text-white ml-4 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2">
                                    EDITAR
                                </a>
                                @csrf
                                @method('DELETE')
