...
<html>
    ...
    <body class="bg-gray-300">
        <div class="w-screen h-screen bg-gray-300">
            <div>
                <h1 class="mt-20 ml-20 mb-4 text-4xl font-extrabold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-6xl">REGISTRO INVENTARIO<mark class="px-2 text-white bg-red-700 rounded ml-3">Suajes</mark></h1>
            </div>
        <div>

        <div class="ml-5 mt-16 mr-20 flex flex-row justify-end">
                @role(['Admin','Supervisor'])
                <a href="inventariosuajes/create" class="text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-[#4285F4]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#4285F4]/55 mr-2 mb-2">
                    ...
                    <span class="ml-2">CREAR NUEVO REGISTRO EN EL INVENTARIO DE SUAJES</span>
                <a>
                @endrole
        ...
        <div class="w-12/12  mt-4 ml-20  flex flex-col items-center">
            <table class="w-11/12 text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                @foreach ( $inventariosuajes as $inventariosuaje )
                        <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700 uppercase">
                            <th scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $inventariosuaje->id }}</th>
                                <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $inventariosuaje->suaje->codigo }}</td>
                                <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $inventariosuaje->contador_uso }}</td>
                                <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $inventariosuaje->fecha_registro }}</td>
                                <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $inventariosuaje->fecha_evento }}</td>
                                <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $inventariosuaje->fecha_baja }}</td>
                                <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $inventariosuaje->historial }}</td>

                                <td>
                                @role(['Admin','Supervisor'])
                                <form action="{{ route ('inventariosuajes.destroy',$inventariosuaje->id)}}" method="POST">
                                <a href="/inventariosuajes/{{ $inventariosuaje->id }}/edit" class="text-white ml-4 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2">
                                    EDITAR
                                </a>
                                @csrf
                                @method('DELETE')
