...
<html>
    <head>
        ...
        <title>RC Tools - Empleados</title>
    </head>
    ...
    <body class="bg-gray-300">
        <div>
            <h1 class="mt-14 ml-20 mb-4 text-6xl font-extrabold leading-none tracking-tight text-gray-900"><mark class="px-2 text-white bg-red-700 rounded ml-3">Empleados</mark></h1>

            <div class="mt-12 mr-20 flex flex-row justify-end">
                @role(['Admin'])
                <a href="empleados/create" class="text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-[#4285F4]/50 font-medium rounded-lg text-sm px-3 py-2.5 text-center inline-flex items-center dark:focus:ring-[#4285F4]/55 mr-2 mb-2">
                <svg class="w-6 h-6 text-white dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <span class="ml-2">REGISTRAR NUEVO EMPLEADO</span>
                </a> 
                @endrole
                        ...
                        <tbody>
                            @foreach ($empleados as $empleado)
                                <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700 uppercase">
                                    ...
                                    <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $empleado->numempleado }}</td>
                                    <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $empleado->nombre }}</td>
                                    <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $empleado->apepaterno }}</td>
                                    <td scope="row" class="px-6 py-4 font-semibold text-xs text-black whitespace-nowrap dark:text-white">{{ $empleado->apematerno }}</td>
                                    ...
                                    <td>
                                        <form action="{{ route ('empleados.destroy',$empleado->id)}}" method="POST">
                                            @role(['Admin'])
                                            <a href="/empleados/{{ $empleado->id }}/edit" class="text-white flex bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 mt-2 py-2.5 mr-2 w-16">
